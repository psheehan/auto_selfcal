...
def gaincal_wrapper(selfcal_library, selfcal_plan, target, band, vis, solint, applymode, iteration, telescope, 
       ...
       for gaintype in np.unique([gaincal_gaintype,'T']):
           for i in range(selfcal_library['spws_set'][vis].shape[0]):  # run gaincal on each spw set to handle spectral scans
              ...
              test_gaincal_return[gaintype] += gaincal(vis=vis,\
                 ...
                 minsnr=gaincal_minsnr if applymode == "calflag" else max(gaincal_minsnr,gaincal_unflag_minsnr), minblperant=4,combine=test_gaincal_combine,\
              ...
              if not os.path.exists('test_inf_EB_'+gaintype+'.g'):
                  cb.open(vis, False, False, False)
                  cb.createcaltable('test_inf_EB_'+gaintype+'.g', 
                          'Complex', 'G Jones', 'spw' in test_gaincal_combine)
                  cb.close()
    ...
    if selfcal_library['obstype'] == 'mosaic' or mode == "cocal":
        ...
        bad = np.where(np.any(flags > 5, axis=tuple(range(len(flags.shape[0:-1])))))[0]
